		...
		if($_POST['size_val']) {
			...
			if($EXTRA_OPTS) :
				...
				foreach($EXTRA_OPTS as $key =>  $EXOP):
					...
					if($EXOP['choose_instrument'] == 'All' ||  $instrument == strtolower($EXOP['choose_instrument']) &&  $size_array[0] == 'All'  ){
					...
					if($instrument == strtolower($EXOP['choose_instrument']) ) :
						if(in_array($_POST['size_val'], $size_array) && $EXOP['choose_instrument'] != 'All') {
							array_push($SIZE_OPTIONS, array('name'=> $EXOP['option_name'], 'price'=> $EXOP['price'], 'arryaof' => 's_'.$instrument, 'acf_feild' => 'extra_optionals' )) ;
						}
					endif;
				endforeach;
	...
	if($_POST['rule'] == 'changedistrict'){
		...
		if($EXTRA_OPTS):
			...
			foreach($EXTRA_OPTS as $key => $EX_OP):
				if($EX_OP['available'][0] == '1' && $EX_OP['choose_instrument'] == 'All'):
					array_push($EXTRA_OPTIONALS, array('name' => $EX_OP['option_name'], 'price' => $EX_OP['price']));
				endif;
			endforeach;
		...
		$JSON_DATA['extra_optionals'] = $EXTRA_OPTIONALS;
		$JSON_DATA['require_book'] 	= $REQUIRE_BOOK;
		$obj =  json_encode($JSON_DATA);
		echo $obj;
		die;
	}
	// end rule changedistrict
	if($_POST['rule'] == 'get_price') {
		...
		if($feild_info['sizes_with_terms']) {
			...
			foreach($feild_info['sizes_with_terms'] as $SD) :
				if($SD['title'] == $tername && $_POST['size_val'] == $SD['size'] ) {
			...
			foreach($_POST['arroff'] as $arrayoff) {
				...
				foreach($EXTRA_OPTS as $key =>  $EXOP):
					...
					if($instrument == strtolower($EXOP['choose_instrument']) || $EXOP['choose_instrument'] == 'All'  ) :
						$keyname = strtolower('s_'.$EXOP['choose_instrument']);
						$size_array = $EXOP[$keyname]; 
						if( $arrayoff == $EXOP['option_name'] && $size_array[0] == 'All'){
		...
		$_SESSION["options_details"] = $optname;
        $tax = $TAX_AMOUNT;
	...
	/*	$add_post = array(
		...
		if($orderID) {
			...
			*/
			...
			update_post_meta( $submission->id, 'order_post', $orderID);
			...
			if(
				...
				update_post_meta( $orderID, $payment_information.'insurance_amount',	'Includes First 3 Months');
			}elseif( 
				$post_d_id == '1213' ||
				$post_d_id == '248' 
			 ) {
				update_post_meta( $orderID, $payment_information.'insurance_amount',	'Includes First 4 Months');
			}else{
				...
				<tr>
					<td>
						...
						$body .= '<tr><td>&nbsp;</td></tr>';
							...
							foreach($_SESSION["options_details"] as $OPS) :
									$body .= '<tr>
										<td style="font-size: 16px; color: blue;font-weight:bold;">'.$OPS['opt_name'].'</td>
										<td align="right" style=" font-size: 16px;font-weight: bold;">$'.$OPS['opt_price'].'</td>
									</tr>';
									$body .= '	<tr><td>&nbsp;</td></tr>';
							endforeach;
						...
						$body .= '<tr><td>&nbsp;</td></tr>';
							foreach($options_array as $OPT) :
								...
								if($OPT == 'Essential Elements Book 1'):
									$options = get_field('essential_elements_', $post_d_id);
								$body .= '<tr>
								...
								if($OPT == 'Suzuki Book'):
									$options = get_field('suzuki_book_', $post_d_id);
									$body .= '<tr>
										<td style="font-size: 16px; color: blue;font-weight:bold;">Suzuki Book</td>
										<td align="right" style=" font-size: 16px;font-weight: bold;">$'.$options['price'].'</td>
									</tr>';
									$body .= '	<tr><td>&nbsp;</td></tr>';
								endif;

								if($OPT == 'Everest Shoulder Rest'):
									$options = get_field('everest_shoulder_rest', $post_d_id);
									
									$body .= '<tr>
										<td style="font-size: 16px; color: blue;font-weight:bold;">Everest Shoulder Rest</td>
										<td align="right" style=" font-size: 16px;font-weight: bold;">$'.$options['price'].'</td>
									</tr>';
									$body .= '	<tr><td>&nbsp;</td></tr>';
								endif;
								...
								if($OPT == 'First Things First'):
									$options = get_field('first_things_first', $post_d_id);
									$body .= '<tr>
										<td style="font-size: 16px; color: blue;font-weight:bold;">First Things First</td>
										<td align="right" style=" font-size: 16px;font-weight: bold;">$'.$options['price'].'</td>
									</tr>';
									$body .= '	<tr><td>&nbsp;</td></tr>';
								endif;
								...
								if($OPT == 'Music Stand'):
									$options = get_field('music_stand', $post_d_id);
									$body .= '<tr>
								...
								if($OPT == 'Measures of Success Vol'):
									$options = get_field('measures_of_success_vol', $post_d_id);
									$body .= '<tr>
										<td style="font-size: 16px; color: blue;font-weight:bold;">Measures of Success Vol</td>
										<td align="right" style=" font-size: 16px;font-weight: bold;">$'.$options['price'].'</td>
									</tr>';
									$body .= '	<tr><td>&nbsp;</td></tr>';
								endif;
							...
							$selected_ins = $submission->form_data_formatted['hidden_input_3'];
							$selected_ins = strtolower($selected_ins);
							
							$extraoptions = get_field('extra_optionals', $post_d_id);

							

							// foreach($extraoptions as $key => $ex){
							// 	//print_r($ex);
							// 	// $matchiing = strtolower($ex['choose_instrument']);
								
							// 	// if(in_array($ex['option_name'], $options_array)) {
							// 	// 	$body .= '<tr>
							// 	// 	<td style="font-size: 16px; color: blue;font-weight:bold;">'.$ex['option_name'].'</td>
							// 	// 		<td align="right" style=" font-size: 16px;font-weight: bold;">$'.$ex['price'].'</td>
							// 	// 	</tr>';
							// 	// 	$body .= '	<tr><td>&nbsp;</td></tr>';
							// 	// }
								
							// 	// if(strtolower($ex['choose_instrument']) == $selected_ins )  {
							// 	// 	//echo $ex['option_name'].'--'.$ex['price'].'<br>';
							// 	// 	$body .= '<tr>
							// 	// 	<td style="font-size: 16px; color: blue;font-weight:bold;">'.$ex['option_name'].'</td>
							// 	// 		<td align="right" style=" font-size: 16px;font-weight: bold;">$'.$ex['price'].'</td>
							// 	// 	</tr>';
